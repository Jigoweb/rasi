import { normalizeTitleStrict } from './title-normalize'
...
function inferSeasonFromTitles(row: Record<string, unknown>): number | null {
  ...
  const escapedOriginal = escapeRegExp(original.trim())
  const suffixMatch = title.match(new RegExp(`^${escapedOriginal}\\s+(\\d{1,2})$`, 'i'))
...
function normalizeEpisodeTitle(value: unknown): string | null {
  ...
  if (!text) return null
  const normalized = normalizeTitleStrict(text.replace(/^["“”]+|["“”]+$/g, '').trim())
