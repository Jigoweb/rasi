-- SKY gap closure (A1/A2 + D2/D3): wire loose match keys + episode ep-only fallback +
-- soft year for film. Based on 20260731160000_matcher_derive_episode_medium_gate body.
-- A1/D2: match-time build_match_key (loose) so "Vikings 5" aligns to catalog "VIKINGS";
--        also candidate via opere.match_key_loose/strict.
-- A2: ep-only fallback even when numero_stagione is set but S+E failed.
-- D3: film no longer hard-skips on year beyond hard tolerance (soft penalty / produzione fallback).
...
DECLARE
    ...
    v_prog_key_loose TEXT;
    v_prog_key_strict TEXT;
    v_prog_key_orig_loose TEXT;
...
BEGIN
    ...
    -- Titolo "pulito" per il matching (A1/D2):
    -- 1) strip ": Season|Parte|…" (c_series_suffix)
    -- 2) loose build_match_key → strip trailing digit/roman/parte ("Vikings 5" → "vikings")
    ...
    v_prog_key_loose := NULLIF(regexp_replace(public.build_match_key(COALESCE(v_prog.titolo, ''), NULL), '::.*$', ''), '');
    v_prog_key_strict := NULLIF(regexp_replace(public.build_match_key_strict(COALESCE(v_prog.titolo, ''), NULL), '::.*$', ''), '');
    v_prog_key_orig_loose := NULLIF(regexp_replace(public.build_match_key(COALESCE(v_prog.titolo_originale, ''), NULL), '::.*$', ''), '');
    IF v_prog_key_loose IS NOT NULL AND v_prog_key_loose <> '' THEN
        v_titolo_match := v_prog_key_loose;
    END IF;
    IF v_prog_key_orig_loose IS NOT NULL AND v_prog_key_orig_loose <> '' THEN
        v_titolo_orig_match := v_prog_key_orig_loose;
    END IF;
    ...
    FOR v_opera IN
        SELECT
            ...
            GREATEST(
                ...
                CASE
                    WHEN v_prog_key_loose IS NOT NULL AND v_prog_key_loose <> ''
                         AND split_part(COALESCE(o.match_key_loose, ''), '::', 1) = v_prog_key_loose
                    THEN 1.0
                    WHEN v_prog_key_loose IS NOT NULL AND v_prog_key_loose <> ''
                         AND split_part(COALESCE(o.match_key_strict, ''), '::', 1) = v_prog_key_loose
                    THEN 1.0
                    WHEN v_prog_key_strict IS NOT NULL AND v_prog_key_strict <> ''
                         AND split_part(COALESCE(o.match_key_strict, ''), '::', 1) = v_prog_key_strict
                    THEN 1.0
                    ELSE 0
                END
            ) as best_score
        ...
        WHERE o.titolo IS NOT NULL
          ...
          AND (
              ...
              -- D2: dual-key exact (year-stripped) — recovers titles where trigram < soglia
              OR (
                  v_prog_key_loose IS NOT NULL AND v_prog_key_loose <> ''
                  AND (
                      split_part(COALESCE(o.match_key_loose, ''), '::', 1) = v_prog_key_loose
                      OR split_part(COALESCE(o.match_key_strict, ''), '::', 1) = v_prog_key_loose
                  )
              )
              OR (
                  v_prog_key_strict IS NOT NULL AND v_prog_key_strict <> ''
                  AND split_part(COALESCE(o.match_key_strict, ''), '::', 1) = v_prog_key_strict
              )
              OR (
                  v_prog_key_orig_loose IS NOT NULL AND v_prog_key_orig_loose <> ''
                  AND (
                      split_part(COALESCE(o.match_key_loose, ''), '::', 1) = v_prog_key_orig_loose
                      OR split_part(COALESCE(o.match_key_strict, ''), '::', 1) = v_prog_key_orig_loose
                  )
              )
          )
    ...
    LOOP
        ...
        -- Film: rilascio vs opera.anno_produzione; soft fallback produzione (no hard scarto).
        -- =====================================================
        v_anno_peso := 15;
        IF v_prog_anno_rilascio IS NOT NULL AND v_opera.anno_produzione IS NOT NULL THEN
            ...
            ELSIF v_score_anno = 0 AND NOT v_is_serie THEN
                -- D3: niente hard-scarto film. Soft fallback produzione (come serie).
                IF v_prog_anno_produzione IS NOT NULL THEN
                    v_score_anno := public.score_year_overlap(
                        v_prog_anno_produzione,
                        v_prog_anno_produzione_fine,
        ...
        IF v_is_serie THEN
            ...
            LOOP
                ...
                BEGIN
                    ...
                    -- A2: ep-only anche se stagione valorizzata ma S+E non ha matchato
                    IF NOT v_match_found AND v_prog.numero_episodio IS NOT NULL THEN
                        IF v_episodio.numero_episodio = v_prog.numero_episodio THEN
                            v_ep_score := GREATEST(
                                v_ep_score,
                                CASE
                                    WHEN v_prog.numero_stagione IS NULL OR v_prog.numero_stagione = 0 THEN 0.8
                                    ELSE 0.75
                                END
                            );
                            v_match_found := TRUE;
