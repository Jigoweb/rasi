-- Harden the delete RPC after optimizing it: authenticated clients must prove
-- the user id they pass is the current JWT subject, while the legacy overload is
-- reserved for server-side service-role callers.
...
REVOKE EXECUTE ON FUNCTION public.delete_campagna_individuazione_safe(uuid) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.delete_campagna_individuazione_safe(uuid, uuid, integer) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.delete_campagna_individuazione_safe(uuid) TO service_role;
GRANT EXECUTE ON FUNCTION public.delete_campagna_individuazione_safe(uuid, uuid, integer) TO authenticated, service_role;
