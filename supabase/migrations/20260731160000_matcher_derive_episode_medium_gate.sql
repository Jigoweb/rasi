-- Wire derive_programmazione_episode_signals into the live year-semantics matcher.
-- Applies high confidence, or medium when a concrete episode number was derived (Ep.N).
-- Does not re-apply older matcher rewrites; based on matcher_year_semantics body.
